...
I wanted to take operations that appear again and again in LLMs, diffusion models, and transformer blocks, write them manually in CUDA, expose them as PyTorch extensions, and then test them properly.

The focus is not just “the kernel compiles.”
...
Input dtype: fp16
Reference: PyTorch eager reference implementations used inside the benchmark file
```
...
The Python tests use contiguous inputs.
...
The GB/s estimate is approximate. Some kernels are not pure memory bandwidth kernels because they also do exp, erf, sqrt, or reduction work.
